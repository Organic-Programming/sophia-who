...
func TestPinVersion(t *testing.T) {
	root := t.TempDir()
	seedHolon(t, root, "pin-uuid-99", "Epsilon")
	...
	if resp.Identity.BinaryVersion != "1.2.3" {
		t.Errorf("BinaryVersion = %q, want %q", resp.Identity.BinaryVersion, "1.2.3")
	}
	if resp.Identity.Os != "linux" {
		t.Errorf("Os = %q, want %q", resp.Identity.Os, "linux")
	}
	if resp.Identity.Arch != "amd64" {
		t.Errorf("Arch = %q, want %q", resp.Identity.Arch, "amd64")
	}
}
...
func TestCreateIdentityAllFields(t *testing.T) {
	...
	if resp.Identity.WrappedLicense != "Apache-2.0" {
		t.Errorf("WrappedLicense = %q, want %q", resp.Identity.WrappedLicense, "Apache-2.0")
	}
...
// --- PinVersion with all fields ---

func TestPinVersionAllFields(t *testing.T) {
	root := t.TempDir()
	seedHolon(t, root, "allpin-uuid", "Zeta")
	...
	if resp.Identity.BinaryPath != "/usr/local/bin/zeta" {
		t.Errorf("BinaryPath = %q", resp.Identity.BinaryPath)
	}
	if resp.Identity.GitTag != "v2.0.0" {
		t.Errorf("GitTag = %q", resp.Identity.GitTag)
	}
	if resp.Identity.GitCommit != "abc123def" {
		t.Errorf("GitCommit = %q", resp.Identity.GitCommit)
	}
}
